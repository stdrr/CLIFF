...
class BaseDataset(Dataset):
    ...
    def __init__(self, dataset, ignore_3d=False, use_augmentation=True, is_train=True,
                normalization=False, cropped=False, crop_size=224, max_cache_size=6e3, start_stop_idxs=None):
        ...
        if start_stop_idxs is not None:
            start_idx, stop_idx = start_stop_idxs
            if start_idx < 0:
                start_idx = len(self.data['imgname']) // abs(start_idx)
            if stop_idx < 0:
                stop_idx = len(self.data['imgname']) // abs(stop_idx)
            elif stop_idx is None:
                stop_idx = len(self.data['imgname'])
            chunk_data = dict()
            for key in self.data.keys():
                chunk_data[key] = self.data[key][start_idx:stop_idx]
    ...
    def load_image(self, imgname, index):
        if imgname not in self._imgs_cache:
    ...
    def __getitem__(self, index):
        ...
        img = self.load_image(imgname, index)
