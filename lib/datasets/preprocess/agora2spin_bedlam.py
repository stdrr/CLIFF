...
sys.path.insert(0, '/media/odin/stdrr/projects/tasks_on_human_meshes/code/CLIFF')

from lib.utils import rotation_conversions as geo
from lib.models.smpl import SMPL
from lib.datasets.preprocess.agora import load_agora_smpl
from lib.datasets.preprocess.agora_projection import *
...
def convert_agora(split_idx):
    for idx in tqdm(range(len(df))):
        for pNum in range(len(df.iloc[idx]['isValid'])):

            sample = df.iloc[idx]
            imgname = join(f'val_{split_idx}', 
...
if __name__ == '__main__':
    # ROOT
    split = 'validation'
    root = '/media/odin/stdrr/data/AGORA/labels'
    smpl = SMPL()
    ...
    for idx in range(10):
        ...
        # DF file
        file = os.path.join(root, f'{split}_SMPL/SMPL/{split}_{idx}_withjv.pkl')
        with open(file, 'rb') as f:
            df = pickle.load(f)

        print(f"Converting {split}_{idx}...")
        convert_agora(split_idx=idx)

    out_file = os.path.join(root, 'spin_format/agora_{split}.npz'.format(split=split))
